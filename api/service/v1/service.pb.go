// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.27.1
// 	protoc        v3.17.3
// source: api/service/v1/service.proto

package v1

import (
	v1 "4clabs-server/api/auth/v1"
	v11 "4clabs-server/api/nft/v1"
	_ "google.golang.org/genproto/googleapis/api/annotations"
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	reflect "reflect"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

var File_api_service_v1_service_proto protoreflect.FileDescriptor

var file_api_service_v1_service_proto_rawDesc = []byte{
	,
}

var file_api_service_v1_service_proto_goTypes = []interface{}{
	(*v1.VerifySignToLoginSignRequest)(nil),  // 0: api.auth.v1.VerifySignToLoginSignRequest
	(*v1.FetchSignMessageRequest)(nil),       // 1: api.auth.v1.FetchSignMessageRequest
	(*v11.GetAddressNftsRequest)(nil),        // 2: api.nft.v1.GetAddressNftsRequest
	(*v11.GetNftDetailRequest)(nil),          // 3: api.nft.v1.GetNftDetailRequest
	(*v1.VerifySignToLoginSighResponse)(nil), // 4: api.auth.v1.VerifySignToLoginSighResponse
	(*v1.FetchSignMessageResponse)(nil),      // 5: api.auth.v1.FetchSignMessageResponse
	(*v11.GetAddressNftResponse)(nil),        // 6: api.nft.v1.GetAddressNftResponse
	(*v11.GetNftDetailResponse)(nil),         // 7: api.nft.v1.GetNftDetailResponse
}
var file_api_service_v1_service_proto_depIdxs = []int32{
	0, // 0: api.service.v1.Nft.SignToLogin:input_type -> api.auth.v1.VerifySignToLoginSignRequest
	1, // 1: api.service.v1.Nft.FetchNonce:input_type -> api.auth.v1.FetchSignMessageRequest
	2, // 2: api.service.v1.Nft.GetAddressNfts:input_type -> api.nft.v1.GetAddressNftsRequest
	3, // 3: api.service.v1.Nft.GetNftDetail:input_type -> api.nft.v1.GetNftDetailRequest
	4, // 4: api.service.v1.Nft.SignToLogin:output_type -> api.auth.v1.VerifySignToLoginSighResponse
	5, // 5: api.service.v1.Nft.FetchNonce:output_type -> api.auth.v1.FetchSignMessageResponse
	6, // 6: api.service.v1.Nft.GetAddressNfts:output_type -> api.nft.v1.GetAddressNftResponse
	7, // 7: api.service.v1.Nft.GetNftDetail:output_type -> api.nft.v1.GetNftDetailResponse
	4, // [4:8] is the sub-list for method output_type
	0, // [0:4] is the sub-list for method input_type
	0, // [0:0] is the sub-list for extension type_name
	0, // [0:0] is the sub-list for extension extendee
	0, // [0:0] is the sub-list for field type_name
}

func init() { file_api_service_v1_service_proto_init() }
func file_api_service_v1_service_proto_init() {
	if File_api_service_v1_service_proto != nil {
		return
	}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: file_api_service_v1_service_proto_rawDesc,
			NumEnums:      0,
			NumMessages:   0,
			NumExtensions: 0,
			NumServices:   1,
		},
		GoTypes:           file_api_service_v1_service_proto_goTypes,
		DependencyIndexes: file_api_service_v1_service_proto_depIdxs,
	}.Build()
	File_api_service_v1_service_proto = out.File
	file_api_service_v1_service_proto_rawDesc = nil
	file_api_service_v1_service_proto_goTypes = nil
	file_api_service_v1_service_proto_depIdxs = nil
}
