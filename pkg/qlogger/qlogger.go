...
var _ logger.Interface = (*DbTraceLogger)(nil)
...
func NewDbTraceLogger(logger *log.Helper) *DbTraceLogger {
	return &DbTraceLogger{slowSqlTime: time.Millisecond * 200, logger: logger}
}

func (d *DbTraceLogger) LogMode(level logger.LogLevel) logger.Interface {
	return d
}

func (d *DbTraceLogger) Info(ctx context.Context, s string, i ...interface{}) {
	d.logger.WithContext(ctx).Infof(s, i)
}

func (d *DbTraceLogger) Warn(ctx context.Context, s string, i ...interface{}) {
	d.logger.WithContext(ctx).Warnf(s, i)
}

func (d *DbTraceLogger) Error(ctx context.Context, s string, i ...interface{}) {
	d.logger.WithContext(ctx).Errorf(s, i)
}

func (d *DbTraceLogger) Trace(ctx context.Context, begin time.Time, fc func() (sql string, rowsAffected int64), err error) {
	...
	case err != nil:
		...
		if !errors.Is(err, gorm.ErrRecordNotFound) {
			...
			d.logger.WithContext(ctx).Warnf("[%.3fms] [rows:%v] %s err for %s", float64(elapsed.Nanoseconds())/1e6, rows, sql, err)
		}
